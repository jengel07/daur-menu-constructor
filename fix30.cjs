...
// fix TS errors
content = content.replace('const groups = {};', 'const groups: Record<string, any[]> = {};');

// Why is groupedArchivedOrders unused? Probably the template replacement failed.
if (!content.includes(`v-else-if="currentTab === 'archived'"`)) {
  console.log("Template replacement actually failed!");
